...
class account_invoice(osv.osv):
    ...
    '''Herencia del modelo account.invoice para agregar campos necesarios en Herrera C.A. '''
    ...
    def get_date_exp(self, cr, uid,ids, payment_id, name):
        res = {}
        #~ payment = self.pool.get('account.payment.term').browse(cr,uid,payment_id).name
        #~ picking_id = self.pool.get('stock.picking.in').search(cr,uid,[('name','=',name)])
        #~ print '>>>>>>>>>>>>>>>>',picking_id
        #~ if payment[0:2].isdigit() or payment[0:1].isdigit():
            #~ val = int(payment[0:2])
        #~ else:
            #~ val = 0   
        #~ if picking_id:
            #~ picking_br = self.pool.get('stock.picking').browse(cr,uid,picking_id)[0]
            #~ date = datetime.datetime.strftime(str_to_datetime(picking_br.date_done[0:10]) + datetime.timedelta(days=val), tools.DEFAULT_SERVER_DATE_FORMAT)
            #~ value = {'date_due': date }
...
class inherited_account_invoice_line(osv.osv):
    ...
    _columns = {
        ...
        'to_refund': fields.boolean('Aplica reembolso', help="Indica si aplica reembolso"),
    }
